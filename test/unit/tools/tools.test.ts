...
test.beforeEach(() => {
	...
	readFileStub
		.withArgs('/testdir/test.js', 'utf-8')
		.resolves('line1\nline2\nline3');
	...
	readFileStub
		.withArgs('/testdir/existing.js', 'utf-8')
		.resolves('existing content');
	writeFileStub
		.withArgs('/testdir/existing.js')
		.rejects(new Error('File already exists, use overwrite=true'));
	...
	accessStub.callsFake(path => {
		...
		if (
			path === '/testdir/test.js' ||
			path === '/testdir/directory' ||
			path === '/testdir/large.js' ||
			path === '/testdir/existing.js'
		) {
...
test('formatToolParams - should format create_file parameters', t => {
	const result = formatToolParams('create_file', {
		file_path: 'new.js',
		content: 'test',
	});
...
test('formatToolParams - should handle arrays with many items', t => {
	const result = formatToolParams('search_files', {
		pattern: 'test',
...
test('formatToolParams - should handle tools without key parameters', t => {
	const result = formatToolParams('create_tasks', {
		user_query: 'test',
		tasks: [],
	});
	t.is(result, '');
...
test('createTasks - should set default status for tasks', async t => {
	const tasks = [{id: '1', description: 'Task 1'} as any];
...
test('createTasks - should validate task structure', async t => {
	const invalidTasks = [{description: 'Missing ID'} as any];
...
test('createTasks - should validate task status', async t => {
	const tasks = [{id: '1', description: 'Task 1', status: 'invalid'} as any];
...
test('updateTasks - should update tasks successfully', async t => {
	...
	const updates = [{id: '1', status: 'completed' as const, notes: 'Done!'}];
...
test('updateTasks - should validate update status', async t => {
	...
	const updates = [{id: '1', status: 'invalid'} as any];
...
test('updateTasks - should handle task not found', async t => {
	...
	const updates = [{id: 'nonexistent', status: 'completed' as const}];
...
test('TOOL_REGISTRY - should contain all expected tools', t => {
	...
	expectedTools.forEach(toolName => {
		...
		t.is(
			typeof TOOL_REGISTRY[toolName as keyof typeof TOOL_REGISTRY],
			'function',
		);
	});
});

test('executeTool2 - should handle invalid arguments', async t => {
...
test('executeTool - should handle type errors', async t => {
	...
	const readFileStub = sinon
		.stub(TOOL_REGISTRY, 'read_file')
		.rejects(new TypeError('Invalid arguments'));
...
test('executeTool - should handle unexpected errors', async t => {
	...
	const result = await executeTool('read_file', {
		file_path: 'nonexistent-error-file.js',
	});
